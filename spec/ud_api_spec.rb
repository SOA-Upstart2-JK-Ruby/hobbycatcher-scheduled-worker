...
describe 'Tests Udemy API library' do
  ...
  before do
    ...
    @course = HobbyCatcher::UdemyApi.new(UDEMY_TOKEN)
                                    .course(COURSEID)
  end
  ...
  describe 'Course' do
    ...
    it 'SAD: should raise exception on incorrect course' do
      _(proc do
        HobbyCatcher::UdemyApi.new(UDEMY_TOKEN).course('wrongid')
      end).must_raise HobbyCatcher::UdemyApi::Errors::NotFound
    end

    it 'SAD: should raise exception when unauthorized' do
      _(proc do
        HobbyCatcher::UdemyApi.new('WrongToken').course('')
      end).must_raise HobbyCatcher::UdemyApi::Errors::Forbidden
    end
  end
